...
hnscHPVbothCombined %>%
  ...
  stat_summary(geom = "text", 
               ...
               colour = "blue",
               position = position_dodge(width = 0.75)) +
  stat_compare_means(comparisons = hnscHPVcomparisons, label.y = 20,
                     label = "p.format") # -> p # Add pairwise comparisons p-value
ggsave(filename = paste0("Figures/",plotPrefix,"/ecologicalVal_HPV_HNSC_p16_ISH.jpeg"),
       width = 4, height = 4.5, units = "in", dpi = "retina")

hnscHPVbothCombined %>%
  filter(sample_type %in% c("Primary Tumor","Blood Derived Normal")) %>%
  filter(!is.na(hpv_status_by_p16_testing)) %>%
  filter(!is.na(hpv_status_by_ish_testing)) %>%
  ggboxplot(x = "testValue", y = "HPV", 
            # color = "testValue",
