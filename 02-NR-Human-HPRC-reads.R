...
# Load count data
countsVb_BIOM <- read_biom(biom_file = "Input_data/against-tcga-kraken1-delta/pangenome-adapter-filter-pese/genus_dbtcga_without_580_and_29_bac_and_vir_NONE_merged.biom")
...
sum(metaDataFilenames %in% rownames(countsVb)) # 15511
sum(rownames(countsVb) %in% metaDataFilenames) # 15511
...
dim(metaDataFinalNonzero) # 15511    41
dim(countsVbFinal) # 15511  1807
...
dim(countsVbFinalNonzeroWIS) # 15511   184
...
metaDataFinalNonzeroQC <- metaDataFinalNonzero %>%
  ...
  droplevels()
dim(metaDataFinalNonzeroQC) # 15179    41
...
source("00-functions.R")
# Note: covDesignNorm had dimensions: 15179    18
...
save(countsVbFinalNonzeroQC,
     ...
     file = paste0("Interim_data/",plotPrefix,"/data_vsnm_tcga_full_and_wis_features_subset_20Feb24.RData"))
...
save(# Subset raw count data
  ...
  file = paste0("Interim_data/",plotPrefix,"/data_per_center_tcga_full_and_wis_features_subset_20Feb24.RData"))
...
save(metaDataFinalNonzeroQCPath,
     ...
     file = paste0("Interim_data/",plotPrefix,"/metadata_stage_noMut_subsets_12Feb24.RData"))
...
countsVbFinalNonzeroQCForm <- countsVbFinalNonzeroQCForm[sampleIDOverlap,featOverlap]
# Realign orig data
vbDataBarnDFReconciledQC_Matched <- vbDataBarnDFReconciledQC[sampleIDOverlap,featOverlap]

dim(countsVbFinalNonzeroQCForm) # 15180  1879
dim(vbDataBarnDFReconciledQC_Matched) # 15180  1879
...
# Sanity check
all(rownames(countsVbFinalNonzeroQCForm) == metaDataFinalNonzeroQC$knightlabID)
